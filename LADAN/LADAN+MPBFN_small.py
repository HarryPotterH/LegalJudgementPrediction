...
from keras.layers import Dense, Input, Flatten, Lambda, Activation, Conv2D, MaxPooling2D, Conv1D, MaxPooling1D, Reshape, \
    Dropout, Average
from keras.layers import average, merge, concatenate, subtract, multiply, dot, add, maximum,Softmax
...
def gen_dict(inputs_, law_labels_input_, accu_labels_input_, time_labels_input_):
    ...
    feed_dict_ = {fact_input: inputs_, law_labels: law_labels_input_,
                  ...
                  law_input_2:law_input_value,
                  accu_input_2:accu_input_value,
                  term_input_2:term_input_value}
...
with tf.Graph().as_default():
    ...
    with tf.name_scope('interaction'):
        ...
        # graph_representation = tf.dynamic_partition(rep_law_1, graph_list, max_graph)
        # graphchoose_list = []
        # for i in range(max_graph):
        #     u = tf.reduce_mean(graph_representation[i], 0, keepdims=True)  # law_representation[i]: [n, law_size]
        #     graphchoose_list.append(u)
        ...
        law_representation = tf.dynamic_partition(law_conv, graph_list,
                                                  max_graph)  # size: [graph_num, n, law_size]
        ...
        for i in range(max_graph):
            ...
            atten_list.append(
                tf.concat([u, u_2], -1))  # size: [graph_num, 2*law_size] whether this u can use attention to get
    ...
    with tf.name_scope('MPBFM'):
        feature = fact_repr
        feature_law = Dense(config_2.hid_size, use_bias=False, activation='relu')(feature)
        law_embedding_layer = tf.get_variable('law_embedding_layer', shape=[config_2.num_law, config_2.hid_size], initializer=tf.contrib.layers.xavier_initializer(), trainable=True)

        accu_embedding_layer = tf.get_variable('accu_embedding_layer', shape=[config_2.num_accu, config_2.hid_size], initializer=tf.contrib.layers.xavier_initializer(), trainable=True)

        term_embedding_layer = tf.get_variable('term_embedding_layer', shape=[config_2.num_term, config_2.hid_size], initializer=tf.contrib.layers.xavier_initializer(), trainable=True)

        law_preds1 = Dense(config_2.num_law, activation='softmax')(feature_law)  # res_law

        law_input_2 = tf.placeholder(shape=(batch_size, config_2.num_law,), dtype=tf.int32)

        law_merge = law_preds1 @ law_embedding_layer  # (130 * 256)
        # law_merge = law_preds1 @ law_embedded_sequences
        law_merge_semantic = Dense(config_2.hid_size, use_bias=False, activation='relu')(law_merge)  # sem

        law_fix = multiply([feature, law_merge_semantic])  # fact_law
        # 针对law任务的fact semantic vector:law_fix

        accu_preds1 = Dense(config_2.num_accu, activation='softmax')(law_fix)  # pred_law == res_accu

        accu_input_2 = tf.placeholder(shape=(batch_size,config_2.num_accu,), dtype=tf.int32)
        accu_merge = accu_preds1 @ accu_embedding_layer
        # accu_merge = accu_preds1 @ accu_embedded_sequences
        accu_merge_semantic = Dense(config_2.hid_size, use_bias=False, activation='relu')(accu_merge)  #sem

        accu_fix = multiply([feature, accu_merge_semantic])  # fact_accu
        # 针对accu任务的fact semantic vector:law_fix

        term_preds1 = Dense(config_2.num_term, activation='softmax')(accu_fix)  # pred_law2penalty
        term_preds2 = Dense(config_2.num_term, activation='softmax')(law_fix)  # pred_accu2penalty

        term_preds3 = multiply([term_preds1, term_preds2])  # res_penalty
        term_preds3 = Lambda(normalize, name="term_preds")(term_preds3)  # y_penalty

        term_input_2 = tf.placeholder(shape=(batch_size, config_2.num_term,), dtype=tf.int32)

        term_merge = term_preds3 @ term_embedding_layer

        # term2law = Dense(config_2.num_law, activation='softmax')(term_merge)  # gate_penalty2law
        # term2accu = Dense(config_2.num_accu, activation='softmax')(term_merge)  # gate_penalty2accu
        # accu2law = Dense(config_2.num_law, activation='sigmoid')(accu_merge)  # gate_accu2law

        term2law = Dense(config_2.num_law, activation='sigmoid')(term_merge)  # gate_penalty2law
        term2accu = Dense(config_2.num_accu, activation='sigmoid')(term_merge)  # gate_penalty2accu
        accu2law = Dense(config_2.num_law, activation='sigmoid')(accu_merge)  # gate_accu2law

        law_preds = Lambda(normalize, name="gate_law_preds")(multiply([term2law, accu2law]))
        law_preds2 = multiply([law_preds1, law_preds])

        # accu_preds2 = multiply([accu_preds1, term2accu])
        accu_preds2 = multiply([accu_preds1, Lambda(normalize, name="gate_accu_preds")(term2accu)])
        accu_preds2 = Lambda(normalize, name="accu_preds")(accu_preds2)

        # accu_preds2 = accu_preds1
    ...
    output_task1, output_task2, output_task3 = law_preds2, accu_preds2, term_preds3

    law_predictions = tf.argmax(output_task1, 1)
    accu_predictions = tf.argmax(output_task2, 1)
    time_predictions = tf.argmax(output_task3, 1)

    law_loss = tf.losses.log_loss(gold_matrix_law, output_task1)
    accu_loss = tf.losses.log_loss(gold_matrix_accu, output_task2)
    time_loss = tf.losses.log_loss(gold_matrix_time, output_task3)

    # loss = (law_loss + accu_loss + time_loss) / batch_size + loss_law_article / 103.0
    loss = (law_loss + accu_loss + time_loss)+ loss_law_article / 103.0 + 0.1 * graph_chose_loss
    # loss = (law_loss + accu_loss + time_loss) + 0.1 * graph_chose_loss
    tf.add_to_collection('losses_1', tf.contrib.layers.l2_regularizer(0.0001)(law_loss))
    tf.add_to_collection('losses_2', tf.contrib.layers.l2_regularizer(0.0001)(accu_loss))
    tf.add_to_collection('losses_3', tf.contrib.layers.l2_regularizer(0.0001)(time_loss))
    ...
    law_input_value = np.tile(np.reshape(np.arange(config_2.num_law), [1, -1]), [batch_size, 1])
    accu_input_value = np.tile(np.reshape(np.arange(config_2.num_accu), [1, -1]), [batch_size, 1])
    term_input_value = np.tile(np.reshape(np.arange(config_2.num_term), [1, -1]), [batch_size, 1])
